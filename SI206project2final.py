...
def grab_headlines():
    ...
    # use this line to run code against test cases
    response = open("opinion.html", "r")
    
    soup = BeautifulSoup(response, "html.parser")
    mr_html = soup.find_all('ol')[0]
    most_read = []
    for tag in mr_html:
        if len(tag.string) > 1:
            most_read.append((tag.string).rstrip())
    return most_read
...
def get_umsi_data():
    ...
    for page in range(13):
        ...
        response = requests.get(current_url, headers={'User-Agent': 'SI_CLASS'})
        soup = BeautifulSoup(response.content, "html.parser")
        raw_people = soup.find_all('div', class_='field field-name-title field-type-ds field-label-hidden')
        raw_titles = soup.find_all('div', class_='field field-name-field-person-titles field-type-text field-label-hidden')
        for x in range(len(raw_people)):
            name = raw_people[x].string
            title = raw_titles[x].string
